...
for (const filePath of sourceFiles) {
  ...
  // Determine relative path from this file to QUAN_TRI_WEBSITE
  const relativeToRoot = path.relative(path.dirname(filePath), __dirname).replace(/\\/g, '/');
  
  for (const [oldName, newName] of Object.entries(regexMapping)) {
    // Look for imports like: import ... from '../data/siteConfig';
    // Replace the path to point to QUAN_TRI_WEBSITE/newName
    
    // Pattern: from '.../data/oldName' or ".../data/oldName"
    const importRegex = new RegExp(`from\\s+['"]([^'"]*)\\/data\\/${oldName}['"]`, 'g');
    content = content.replace(importRegex, `from '${relativeToRoot}/QUAN_TRI_WEBSITE/${newName}'`);
  }
